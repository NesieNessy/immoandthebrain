...
export default function ServiceChargeAdjustmentPage({ propertyId, unitId }: { propertyId: string; unitId: string }) {
    ...
    return (
        <ServiceChargeAdjustmentContent
...
function ServiceChargeAdjustmentContent({
    propertyId,
    property,
    unit,
    hasMultipleUnits,
    ...
    useEffect(() => {
        if (view === 'preview' && data.previewAdjustmentHtml === null && data.canGenerateAdjustmentDocx) {
            void data.handlePreviewAdjustment();
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [view, data.canGenerateAdjustmentDocx]);
    ...
    const breadcrumbItems: BreadcrumbItem[] = [
        ...
        { label: 'Anpassungsschreiben generieren' },
    ];
    ...
    const outcomeLabel = data.settlementCoverage === 'shortfall' ? 'Nachzahlung' : data.settlementCoverage === 'surplus' ? 'Erstattung' : 'Ausgeglichen';
        ...
        <div className="min-h-screen bg-background pb-24">
            <main className={PAGE_CONTAINER_CLASS}>
                ...
                <div className="flex items-center gap-2 p-1 rounded-lg bg-muted/50 w-fit">
                    ...
                    <button
                        type="button"
                        onClick={() => data.canGenerateAdjustmentDocx && setView('preview')}
                ...
                {view === 'review' ? (
                    <div className="mt-6 flex flex-col gap-6">
                        <div className="flex items-start gap-2.5 p-3 rounded-lg border border-border bg-muted/30">
                            <Icons.FileSignature className="w-4 h-4 shrink-0 text-primary mt-0.5" />
                            <p className="text-sm text-muted-foreground">
                                Das Anpassungsschreiben wird automatisch aus dem Abrechnungsergebnis und der neuen NK-Vorauszahlung befüllt. Prüfe die Angaben, bevor du das Dokument generierst.
                            </p>
                        </div>
                        ...
                        <div>
                            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Abrechnungsergebnis</p>
                            <DataCard icon={Icons.FileSignature} title="Anpassungsschreiben" source="Bestandsobjekt · Nebenkostenabrechnung">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <Field label="Ergebnis" value={outcomeLabel} />
                                    <Field label="Betrag" value={euro(Math.abs(data.overUnderCoverage))} />
                                    <Field label="Neue NK-Vorauszahlung" value={data.newMonthlyPrepayment != null ? `${euro(data.newMonthlyPrepayment)}/Monat` : '–'} />
                                </div>
                            </DataCard>
                        </div>
                    </div>
                    ...
                    <div className="mt-6">
                        {data.canGenerateAdjustmentDocx ? (
                            data.previewAdjustmentHtml ? (
                                <iframe
                                    title="Anpassungsschreiben Vorschau"
                                    srcDoc={data.previewAdjustmentHtml}
            ...
            <StickyActionBar
                ...
                onPrimary={() => void data.handleGenerateAdjustmentDocx()}
                ghostLabel={BUTTON_DETAILS.Back.label}
                ghostIcon={<BUTTON_DETAILS.Back.icon />}
                primaryLabel={data.isGeneratingAdjustmentDocx ? 'Wird erstellt…' : 'Word-Dokument generieren'}
                primaryIcon={<Icons.FileSignature className="w-4 h-4" />}
                primaryDisabled={!data.canGenerateAdjustmentDocx || data.isGeneratingAdjustmentDocx}
            />
        </div>
    );
}
